...
const ProjectDetail: React.FC = () => {
    ...
    useEffect(() => {
        ...
        supabase.from("project_parts").select("part_id").eq("project_id", projectId).then(({data}) => setProjectPartIds(data?.map(r => r.part_id) || []));
    }, [currentOrg, projectId]);

    const handleRemoveFromProject = async (part: PartWithDocument) => {
        const {error} = await supabase.from("project_parts").delete().eq("project_id", projectId).eq("part_id", part.id);
        if (!error) setProjectPartIds(prev => prev.filter(id => id !== part.id));
    };

    const projectParts = useMemo(() => parts.filter(p => projectPartIds.includes(p.id)), [parts, projectPartIds]);

    return (
        <div className="p-6 mx-auto" style={{maxWidth: "1800px"}}>
            <div className="flex items-center gap-4 mb-6">
                <button onClick={() => navigate(`/${lang}/app/projects`)}
                        className="p-2 border rounded-md hover:bg-gray-50 transition-colors">
                    <ArrowLeft className="w-4 h-4"/>
                </button>
                <h1 className="text-xl font-bold text-gray-900">{project?.name || t("projects.detail.projectFallback")}</h1>
            </div>
