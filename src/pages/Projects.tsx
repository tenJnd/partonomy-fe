...
const Projects: React.FC = () => {
    ...
    // --- URL helpery (stejný pattern jako PartsManager) ---
    ...
    const [statusFilter, setStatusFilter] = useState<StatusFilter>(
        () => getParam("st", "all") as StatusFilter
    );
    const [priorityFilter, setPriorityFilter] = useState<PriorityFilter>(
        () => getParam("pr", "all") as PriorityFilter
    );

    const [sortField, setSortField] = useState<ProjectSortField>(
        () => getParam("sort", "created_at") as ProjectSortField
    );
    const [sortDirection, setSortDirection] = useState<"asc" | "desc">(
        () => (getParam("dir", "desc") as "asc" | "desc")
    );

    // sync při back/forward (volitelné, ale praktické)
    useEffect(() => {
        setStatusFilter(getParam("st", "all") as StatusFilter);
        setPriorityFilter(getParam("pr", "all") as PriorityFilter);
        setSortField(getParam("sort", "created_at") as ProjectSortField);
        setSortDirection(getParam("dir", "desc") as "asc" | "desc");
    ...
    const handleSortChange = (field: ProjectSortField) => {
        ...
        setParam({sort: field, dir: nextDir});
    };
                                    ...
                                    <input
                                        ...
                                        defaultValue={
                                            editingProject?.due_date
                                                ? new Date(editingProject.due_date).toISOString().slice(0, 10)
                                                : ""
                                        }
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500"
                                    />
