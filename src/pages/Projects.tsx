...
const Projects: React.FC = () => {
    ...
    const [sortField, setSortField] =
        useState<ProjectSortField>("created_at");
    ...
    const handleSubmit = async (
        ...
        const customer_name =
            ((formData.get("customer_name") as string) || "").trim() ||
            null;
        const external_ref =
            ((formData.get("external_ref") as string) || "").trim() || null;
        const status = formData.get("status") as ProjectRow["status"];
        const priority = formData.get("priority") as ProjectRow["priority"];
    ...
    const handleProjectStatusChange = async (
        ...
        setUpdatingStatusIds((prev) => {
            ...
            next.add(project.id as string);
            return next;
        });

        try {
            await updateProject(project.id as string, {status: value});
        } catch (err) {
            console.error("[Projects] update status failed:", err);
        } finally {
            setUpdatingStatusIds((prev) => {
                const next = new Set(prev);
    ...
    // 🔁 změna priority přímo z tabulky
    const handleProjectPriorityChange = async (
        project: ProjectRow,
        ...
        try {
            await updateProject(project.id as string, {priority: value});
        } catch (err) {
            console.error("[Projects] update priority failed:", err);
        } finally {
            setUpdatingPriorityIds((prev) => {
                const next = new Set(prev);
                next.delete(project.id as string);
    ...
    // 🔒 pokud uživatel nemá Projects v plánu
    if (!canUseProjects) {
        return (
            <div className="min-h-screen relative bg-gradient-to-br from-slate-50 via-white to-slate-50">
                ...
                Upgrade
                    ...
                    <div
                        className="max-w-xl bg-white border border-dashed border-gray-200 rounded-lg px-4 py-4 flex gap-3 items-start">
                        <AlertCircle
                        ...
                        <div className="text-sm text-gray-700 space-y-1">
                            ...
                            <p className="text-xs text-gray-600">
                                Use projects to group related parts, RFQs and customers into
                                one view. Go to{" "}
                                <Link
                                    to="/settings/billing"
        ...
        <div className="min-h-screen relative bg-gradient-to-br from-slate-50 via-white to-slate-50">
            <div className="p-6 mx-auto" style={{maxWidth: "1600px"}}>
                ...
                <ProjectsTable
                    ...
                    onChangePriority={handleProjectPriorityChange}
            ...
            {formOpen && (
                <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 relative">
                        ...
                        {formError && (
                            <div
                                className="mb-4 p-3 bg-rose-50 border border-rose-200 rounded-lg flex items-start gap-2">
                                <AlertCircle
